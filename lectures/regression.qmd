...
- Demonstrate that your project can be finished within a month, including statement of **data availability**
...
- How do you infer the relationship between the parental age and the rate of *de novo* mutations?
...
Here $\mathbf{X}$ is a $n \times p$ structure matrix, $\beta$ is a p-dimensional vector with the parameters of our model (a.k.a. regression coefficients), and $\mathbf{\epsilon} = \left( \epsilon_1, \epsilon_2, ... \epsilon_n \right)^\top$ is the noise present in the model.
...
$$ \mathbf{y} = m_1 \mathbf{x_1} + m_2 \mathbf{x_2} \ldots + \beta_0 + \mathbf{\epsilon} $$
...
Consider including the intercept $\beta_0$ by setting $\beta = 
\left(\beta_0 \beta \right)^\top$.
...
$$ \mathbf{y} = m \mathbf{x} + \beta_0 + \mathbf{\epsilon} $$
...
$$\hat{\beta}\sim{}N\left(\beta,\sigma^2 \mathbf{S}^{-1}\right)$$
...
## Ordinary Least Squares method gives the MLE estimate

### Likelihood of Model

$$ \log L(\beta,\sigma^{2}) = -\frac{n}{2} \log(2\pi\sigma^{2}) - \frac{1}{2 \sigma^{2}} \sum_{i=1}^{n} (y_{i}-x_{i}^\top \beta)^{2} $$

therefore, only considering $\beta$ (the only factor that influences predictions), we need to minimize:

$$ \sum_{i=1}^{n} (y_{i}-x_{i}^\top \beta)^{2} $$
...
$$ \ln L(\beta, \sigma^2 \mid y) = -\frac{n}{2}\ln(2\pi\sigma^2) - \frac{1}{2\sigma^2} (y - X\beta)^\top(y - X\beta). $$
:::
...
for the same construction of $\mathbf{\epsilon} \sim \left( \mathbf{0}, \sigma^2 \mathbf{I} \right)$.
...
- In many cases linear regression can be inappropriate
	...
	- E.g. A binary outcome measurements
...
$$
p(x) = \frac{1}{1 + \exp\left(-\left(\beta_0 + \beta^{\top}x\right)\right)}
$$

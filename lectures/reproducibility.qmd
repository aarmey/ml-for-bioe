...
![ ](./figs/reproducibility/title.png){fig-alt="Title slide image for the Genomics Annotations example."}

## Example - Genomics Annotations

![ ](./figs/reproducibility/fig1.png){fig-alt="Figure 1 of the Genomics Annotations example, illustrating initial data or concepts."}

## Example - Genomics Annotations

![ ](./figs/reproducibility/fig2.png){fig-alt="Figure 2 of the Genomics Annotations example, showing further analysis or results."}

## Example - Genomics Annotations

![ ](./figs/reproducibility/fig3.png){fig-alt="Figure 3 of the Genomics Annotations example, likely showing results or a comparison."}

## Example - Genomics Annotations

![ ](./figs/reproducibility/fig4.png){fig-alt="Figure 4 of the Genomics Annotations example, concluding the analysis or showing final results."}
...
- Other source of variation: the code surrounding your analysis
	...
	- A solution to this is `uv`, a modern Python package manager
		- Creates an isolated virtual environment for your project
		- Pins exact versions of all dependencies in a lockfile (`uv.lock`)
		- Much faster than older tools like `pip` or `virtualenv`
	- Projects using `uv` have a `pyproject.toml` file
		- Declares the project's dependencies and metadata
		- The `uv.lock` lockfile records exact versions of every package, ensuring anyone running the project gets the same environment
...
- **Dev containers**: VS Code Dev Containers and GitHub Codespaces provide a more accessible entry point
	- Define your entire development environment in a `.devcontainer/` config file
	- Anyone opening the project gets the same environment automatically
...
- `ruff` is the modern standard for Python linting and formatting
	- Replaces both `pylint` (linter) and `black` (formatter) in a single, extremely fast tool
	- Catches style issues, unused imports, potential bugs, and enforces consistent formatting
...
- Many ML workflows involve randomness: weight initialization, data shuffling, dropout, etc.
	- Setting a random seed (`numpy.random.seed`, `torch.manual_seed`) controls this
	- But GPU operations can still be non-deterministic even with seeds set
...
- Best practices:
	...
	- `nbstripout` removes output and execution counts from notebooks before committing to git
	- `papermill` can execute notebooks programmatically as part of a pipeline
...
- [`unittest`](https://docs.python.org/3/library/unittest.html), the built-in python testing framework
...
- [`ruff`](https://docs.astral.sh/ruff/), a modern python linter and formatter
- [`uv`](https://docs.astral.sh/uv/), a modern Python package and environment manager
- [`dvc`](https://dvc.org), data version control for ML datasets and model artifacts
- [`papermill`](https://papermill.readthedocs.io/), parameterizing and executing Jupyter notebooks

...
- The lecture is recorded via BruinCast to accomondate personal circumstances.
...
- GenAI tools helps you perform the tasks you're familiar with, but mere reliance on it without critical assessment of their output may bloack your learning process
- We will ask you to (1) **declar** the usage of those tools and (2) how did you **verify** the outputs in assignments
...
![Lazebnik et al, Cancer Cell, 2002](./figs/lec1/laz2.jpg){width=2in fig-alt="Photo of inside a radio."}
...
- Possible outcomes ($\Omega$) is $\{1, 2, 3, 4, 5, 6\}$
...
~ The measure of an event given that another event has occured. $p(X \mid Y) = p(X, Y)/p(Y)$.

Marginal distribution
~ The probability distribution regardless of other observations/factors. $p(X) = \sum_{Y}p(X, Y)$.

Complementary event
~ The probability of an event not occuring. $p(X^c) = 1 - p(X)$.
...
Bernoulli
~ Like a coin flip. Binary outcome. A special case of Binomial.
...
::: {.notes}
- For a standard normal distribution, Mean = 0, variance = 1, skewness = 0, kertosis = 3.
:::
...
3. **Central Limit Theorem**: As $n$ increases, sampling distributions of the sample mean tends towards normality. $\bar{X} \approx \mathcal{N}\left(\mu, \frac{\sigma^2}{n}I\right)$

::: {.notes}
- Unbiased estimates vs. biased estimates.
:::

## Sample statistics of the mean
...
The t-distribution with $n-1$ degrees of freedom is useful for comparing means when the sample size $n$ is small and the population variance is unknown.

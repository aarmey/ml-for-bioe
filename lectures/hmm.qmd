...
author: Aaron Meyer (based on notes from Aarti Singh)
---
...
![ ](./figs/hmm/2.pdf){width=80% fig-alt="Diagram representing a Markov chain where each state depends only on the previous state."}
...
![ ](./figs/hmm/4.svg){fig-alt="Graphical model of a Hidden Markov Model showing hidden states S and observed variables O, with arrows indicating dependencies."}
...
![ ](./figs/hmm/6.pdf){fig-alt="State transition diagram of a Hidden Markov Model, showing transition probabilities between states and emission probabilities for observations."}
...
![ ](./figs/hmm/9.pdf){fig-alt="Diagram of the Dishonest Casino example, illustrating the transition between a Fair die and a Loaded die."}

# State Space Representation

Switch between **F** and **L** once every 20 turns (1/20 = 0.05)

![ ](./figs/hmm/10.pdf){fig-alt="State space representation of the Fair and Loaded die switching model."}
...
![ ](./figs/hmm/13.pdf){fig-alt="Visual representation of the Forward Algorithm on a trellis diagram."}

# Forward Probability

![ ](./figs/hmm/14.pdf){fig-alt="Detailed step of the Forward probability calculation."}
...
![ ](./figs/hmm/16.pdf){fig-alt="Visual representation of the Backward Algorithm."}

# Backward Probability

![ ](./figs/hmm/17.pdf){fig-alt="Detailed step of the Backward probability calculation."}
...
![ ](./figs/hmm/20.pdf){fig-alt="Illustration of the Viterbi algorithm finding the most likely sequence of hidden states."}

# Viterbi Decoding

![ ](./figs/hmm/21.pdf){fig-alt="Viterbi decoding process."}

# Viterbi Algorithm

Can compute $V_t^k\;\;\forall k, t$ using dynamic programming:

![ ](./figs/hmm/22.pdf){fig-alt="Visual representation of the Viterbi calculation step."}
...
![ ](./figs/hmm/25.pdf){fig-alt="Diagram illustrating the Baum-Welch (EM) algorithm for HMM parameter learning."}

# Baum-Welch (EM) Algorithm

- Start with random initialization of parameters

![ ](./figs/hmm/26.pdf){fig-alt="Process flow of the Baum-Welch algorithm starting from random initialization."}
...
![ ](./figs/hmm/27.pdf){fig-alt="Diagram showing the connection between HMMs and Dynamic Mixture Models."}
...
![ ](./figs/hmm/gupta1.pdf){fig-alt="Figure from Gupta et al. on Cancer Stem Cells."}

# Example: Cancer Stem Cells

![ ](./figs/hmm/gupta3.pdf){fig-alt="Figure from Gupta et al. on Cancer Stem Cells."}

# Example: Cancer Stem Cells

![ ](./figs/hmm/gupta4.pdf){fig-alt="Figure from Gupta et al. on Cancer Stem Cells."}

# Example: Cancer Stem Cells

![ ](./figs/hmm/gupta5.pdf){fig-alt="Figure from Gupta et al. on Cancer Stem Cells."}

# Example: Cancer Stem Cells

![ ](./figs/hmm/gupta6.webp){fig-alt="Figure from Gupta et al. on Cancer Stem Cells."}

# Example 2: Sequence Prediction

![AXL prediction](./figs/hmm/AXL-THMM.svg){fig-alt="Genomic track view showing AXL gene prediction using a Threshold HMM (THMM) compared to reference annotations and conservation scores."}
...
10. TMHMM is a HMM model that can be used to predict the transmembrane regions of proteins (there’s an example input you can use in the guide). Show us the output. Which algorithm is being used for each part of the output graphic?

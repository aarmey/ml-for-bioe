...
![Model visualization of pneumothorax detection. (a) The model correctly identifies pneumothorax in the right and left upper lungs. (b) The model correctly identifies pneumothorax in the right lower lung. Figure from [Taylor et al., *PLOS Medicine* (2018)](https://journals.plos.org/plosmedicine/article?id=10.1371/journal.pmed.1002686).](figs/nn/pmed.1002686.g003.webp){#fig-pneumothorax}

::: {.notes}
Deep learning has revolutionized medical imaging. In this example, a convolutional neural network was trained to detect pneumothorax (collapsed lung) from chest X-rays. The heatmaps (right) show where the model is "looking" to make its decision, which aligns with the radiologist's findings (left). This ability to automatically extract complex features from raw pixels is what makes neural networks so powerful for biological and medical data.
:::

### Why can't simpler models handle this?

- The feature space is **high-dimensional**: thousands of raw pixel intensities
- The relevant structure is **nonlinear**: pathological state isn't linearly separable in raw pixel space
...
- Recall from the autodifferentiation lecture:
  - **Reverse mode**: one forward pass + one backward pass computes the gradient w.r.t. _all_ parameters in $O(1)$ function evaluations
...
This is the key message of this section. Double descent and UAT explain _why_ overparameterized networks can generalize, but they don't tell you how to _make them_ generalize in practice. That requires the techniques in this section.
:::
...
- This is flexible, but expensive: it must _learn_ from scratch that nearby pixels are correlated, that molecules are rotation-invariant, that sequences have order
...
::: {.notes}
This is the deepest conceptual point of the lecture. UAT says any function is representable; inductive biases say which function is _easy_ to learn. The history of deep learning is largely a history of discovering the right inductive biases for each domain.
...
![The convolution operation. A filter (or kernel) slides across the input image, computing a weighted sum of the local receptive field at each position to produce an activation map. Image credit: Bill Kromydas.](figs/nn/CNN.png){#fig-cnn}
...
![Examples of graph-structured data. Biology is full of relational structure, from molecular bonds and gene regulatory networks to the connectome. Image credit: Rick Merritt.](figs/nn/example-graphs-Jure.jpg){#fig-example-graphs}
...
![The attention mechanism. Input tokens are transformed into Queries ($Q$), Keys ($K$), and Values ($V$). The attention weights are computed by comparing Queries with Keys, then used to take a weighted average of Values. Image credit: Ebrahim Pichka.](figs/nn/attention.png){#fig-attention}
...
![Cell Painting workflow. (a) Genetic or chemical perturbations are applied to cells in multi-well plates. (b) Cells are stained and imaged via microscopy. (c) Automated image analysis extracts morphological features. (d) Resulting morphological profiles are used for downstream tasks. Figure from [Bray et al., *Nature Protocols* (2016)](https://pubmed.ncbi.nlm.nih.gov/27560178/).](figs/nn/cell-painting.jpg){#fig-cell-painting}

::: {.notes}

- **Cell Painting** ([Bray et al., 2016](https://pubmed.ncbi.nlm.nih.gov/27560178/)): image cells stained with 6 fluorescent dyes; extract ~1,500 morphological features
...
The key insight is that cell morphology is a rich, high-dimensional phenotype that encodes biological state. Neural networks, especially with CNN inductive biases, can extract this information in ways that hand-crafted features cannot.
:::
...
- LeCun Y., Bengio Y., Hinton G. [Deep learning](https://www.nature.com/articles/nature14539). _Nature_, 2015.
- Srivastava N. _et al._ [Dropout: A Simple Way to Prevent Neural Networks from Overfitting](https://jmlr.org/papers/v15/srivastava14a.html). _JMLR_, 2014.
- Jumper J. _et al._ [Highly accurate protein structure prediction with AlphaFold](https://www.nature.com/articles/s41586-021-03819-2). _Nature_, 2021.
- Ching T. _et al._ [Opportunities and obstacles for deep learning in biology and medicine](https://doi.org/10.1098/rsif.2017.0387). _J. R. Soc. Interface_, 2018.

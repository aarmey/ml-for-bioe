...
$$\frac{dR_b}{dt} = k_2 L_D R_f - k_{-2} R_b, \quad \frac{dR_f}{dt} = k_{-2} R_b - k_2 L_D R_f$$
...
Governing equations for the two-compartment model:
...
![](./figs/dynamics/scipy-integrate-odeint-1.png){fig-alt="Plot of pendulum angle and angular velocity over time" .r-stretch}
...
For a diagonalizable $J = V \Lambda V^{-1}$, this becomes:

$$e^{Jt} = V e^{\Lambda t} V^{-1}$$

where $e^{\Lambda t}$ is a diagonal matrix of $e^{\lambda_i t}$ — directly connecting to our eigenvalue stability analysis.

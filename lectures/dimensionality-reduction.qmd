...
![Visualization of matrix factorization, showing a data matrix X approximated by the product of W and H transpose, with sample and feature axes and common names for the factor matrices in PCA and NMF.](./figs/dimred/mflayout-tikz.svg)
...
::: {.notes}
- Good place to pause and ask whether gene expression counts, heights, and blood pressure should be compared on raw scales.
- If the question is about total magnitude, maybe do not scale.
- If the question is about patterns across features, scaling is often much more sensible.
:::
...
![ ](./figs/dimred/PCA-Fig1a.svg){fig-alt=""}
...
![ ](./figs/dimred/PCA-Fig1b.svg){fig-alt=""}

---

### Scores for each data point

![ ](./figs/dimred/PCA-Fig1c.svg){fig-alt=""}
...
![ ](./figs/dimred/PCA-Fig2.svg){fig-alt=""}
...
![ ](./figs/dimred/PCA-Fig3a.svg){fig-alt=""}
...
![ ](./figs/dimred/PCA-Fig3b.svg){fig-alt=""}
...
![Visualization of PCA as a low-rank approximation: data points are approximated by projection onto the leading principal component directions.](./figs/dimred/ElemStatLearnII_fig14.21.svg){width=90%}

::: {.notes}
- This is the visual bridge between "variance-maximizing directions" and "low-rank matrix approximation."
- The approximation is not arbitrary: among all rank-$R$ approximations, PCA/SVD gives the one with the smallest squared reconstruction error.
- Nice place to say: compress, then reconstruct.
:::
...
- Help students connect "points" to scores and "arrows/weights" to loadings.
- Stress that opposite signs are relative within a component, not good/bad.
:::
...
::: {.notes}
- There is no single universally correct cutoff.
- Distinguish exploratory visualization from building a predictive pipeline.
:::
...
- Implemented within `sklearn.decomposition.PCA`
	...
	- `PCA.explained_variance_ratio_` gives the fraction of total variance explained by each PC
...
\mathbf{W}_{ij} \ge 0,\ \mathbf{H}_{ij} \ge 0
$$

Typically, we minimize reconstruction error:

$$
\min_{\mathbf{W},\mathbf{H}} \left\|\mathbf{X} - \mathbf{W}\mathbf{H}^\top\right\|_F^2
\qquad
\text{subject to } \mathbf{W}, \mathbf{H} \ge 0
$$

::: {.notes}
- Good place to remind students that this looks like PCA's reconstruction objective, but now with non-negativity constraints.
- Can mention that other losses are also possible, but squared error is the standard starting point.
:::
...
Color indicates the [data term]{style="color:darkred;"} and the [factor term]{style="color:darkblue;"}.

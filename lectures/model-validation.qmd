...
![ ](./figs/crossval/sfd-5.png){fig-alt="Plot showing the relationship between model complexity and prediction error. Training error decreases with complexity, while test error follows a U-shape, indicating overfitting at high complexity."}
...
![ ](./figs/crossval/sfd-8.png){fig-alt="Diagram illustrating the validation set approach, where the dataset is randomly split into a training set and a validation set."}
...
![ ](./figs/crossval/sfd-9.png){fig-alt="Plots showing the Mean Squared Error (MSE) versus polynomial degree for the automobile dataset. The left panel shows a single split, while the right panel shows variability across multiple random splits."}
...
Table: Divide data into $K$ roughly equal-sized parts ($K = 5$ here). Each row shows which part is held out for that fold.
...
- Compute: $$CV_{(K)} =  \sum^{K}_{k=1} \frac{n_k}{n} MSE_k$$
	...
	- $\hat{y}_i$ is the prediction for observation $i$ when its fold was removed from training.
- Setting $K = n$ yields *leave-one out cross-validation* (LOOCV). LOOCV uses nearly all the data for training, minimizing bias, but the estimates across folds come from nearly-identical training sets and are therefore highly correlated — leading to high variance in the overall error estimate.

## Auto data revisited

![ ](./figs/crossval/sfd-18.png){fig-alt="Comparison of Mean Squared Error (MSE) estimates using Leave-One-Out Cross-Validation (LOOCV) and 10-fold Cross-Validation on the automobile dataset."}

## True and estimated test MSE for the simulated data

![ ](./figs/crossval/sfd-19.png){fig-alt="Plot comparing the true test MSE with estimates from 10-fold Cross-Validation and LOOCV on simulated data."}
...
- When you tune hyperparameters *and* want an honest estimate of generalization error, a single CV loop does both — but the chosen model was selected based on the same folds used to estimate error, producing an optimistic bias.
- **Nested CV** separates the two concerns:
	- **Outer loop**: holds out a test fold to estimate generalization error.
	- **Inner loop**: runs a full CV on the remaining data to select hyperparameters.
- The outer error estimate is unbiased because the test fold never influenced model selection.
- Computationally expensive ($K_{\text{outer}} \times K_{\text{inner}}$ fits), but essential when comparing models that were tuned to the data.

::: {.notes}
Draw the nested loop on the board. Emphasize: the outer fold is touched exactly once, only for evaluation.
:::

# The Bootstrap

- Cross-validation resamples by *partitioning* — it asks how well the model generalizes to held-out data.
- The bootstrap resamples *with replacement* and serves a complementary purpose: quantifying the uncertainty of an estimate itself.
...
![ ](./figs/crossval/sfd-43.png){fig-alt="Illustration of the bootstrap resampling method on a small dataset with three observations."}
...
![ ](./figs/crossval/sfd-45.png){fig-alt="Schematic diagram of the bootstrap process, showing the generation of bootstrap samples, calculation of statistics, and estimation of standard error."}
...
- But each bootstrap sample has significant overlap with the original data. About two-thirds of the original data points appear in each bootstrap sample. The probability that a given observation is *not* selected in a single draw is $(1 - 1/n)$. Over $n$ draws, the probability it never appears is $(1 - 1/n)^n \to e^{-1} \approx 0.368$ as $n \to \infty$, so roughly $1 - 0.368 = 63.2\%$ of observations appear in each bootstrap sample.
...
- The following papers illustrate the real consequences of CV leakage in biomedical research.
- Gene selection (step 1) was performed on the full dataset before cross-validation (step 2) — exactly the error described above.
- The result: signatures built from *random, biologically meaningless genes* appeared prognostically significant.

![ ](./figs/crossval/example-1.png){fig-alt="Survival curves showing prognostic gene expression signatures separating patients into risk groups."}

## Random gene expression signatures are associated with survival

![**Figure 1. Association of negative control signatures with overall survival.** In plots A-C the NKI cohort was split into two groups using a signature of post-prandial laughter (panel A), localization of skin fibroblasts (panel B), social defeat in mice (panel C). In panels A-C, the fraction of patients alive (overall survival, OS) is shown as a function of time for both groups. Hazard ratios (HR) between groups and their associated p-values are given in bottom-left corners. Panel D depicts p-values for association with outcome for all MSigDB c2 signatures and random signatures of identical size as MSigDB c2 signatures.](./figs/crossval/pcbi.1002240.g001.png){fig-alt="Figure 1 from the paper, showing Kaplan-Meier survival curves for negative control signatures (random events) which still show significant separation."}

## Random signatures often do better than the published ones

![**Figure 2. Most published signatures are not significantly better outcome predictors than random signatures of identical size.** The x-axis denotes the p-value of association with overall survival. Red dots stand for published signatures, yellow shapes depict the distribution of p-values for 1000 random signatures of identical size, with the lower 5% quantiles shaded in green and the median shown as black line. Signatures are ordered by increasing sizes.](./figs/crossval/pcbi.1002240.g002.png){fig-alt="Figure 2 showing that most published signatures do not perform significantly better than random signatures of identical size."}

## Many signatures simply correlate with cell proliferation

![**Figure 4. Most prognostic transcriptional signals are correlated with meta-PCNA.** A) Each point denotes a signature. The x-axis depicts the absolute value of the correlation of the first principal component of the signatures with meta-PCNA, the y-axis depicts the hazard ratio for outcome association. Details of the analysis for each data point are available in the Supporting Information (Text S1). B) Distribution of the correlations of individual genes with meta-PCNA, for genes significantly associated with overall survival (red) and for all the genes spotted on the microarrays (black).](./figs/crossval/pcbi.1002240.g004.png){fig-alt="Figure 4 showing the correlation of prognostic signatures with cell proliferation (meta-PCNA)."}
...
Use `cross_val_predict` instead when you need the out-of-fold predictions themselves (e.g., for calibration plots or stacking) rather than just a summary score.
...
- `sklearn.model_selection.StratifiedKFold(n_splits=5)` — preserves class proportions in each fold; important for imbalanced classification problems where an unlucky split could place all minority-class examples in training or validation.

All use loop `for train_index, test_index in kf.split(X):`.

...
- As usual, we assume the model: $$y=f(\mathbf{z})+\epsilon, \epsilon\sim \mathcal{N}(0,\sigma^2I)$$
...
- To answer this question, we might consider the **mean squared error** of our estimate $\hat{\boldsymbol\beta}$:
	- i.e., consider squared distance of $\hat{\boldsymbol\beta}$ to the true $\boldsymbol\beta$: $$\textrm{MSE}(\hat{\boldsymbol\beta}) = \mathop{\mathbb{E}}\left[\lVert \hat{\boldsymbol\beta} - \boldsymbol\beta \rVert^2\right] = \mathop{\mathbb{E}}[(\hat{\boldsymbol\beta} - \boldsymbol\beta)^\intercal (\hat{\boldsymbol\beta} - \boldsymbol\beta)]$$
- **Example:** In least squares (LS), we know that: $$\mathrm{MSE}(\hat{\boldsymbol\beta}^{ls}) = \mathop{\mathbb{E}}[(\hat{\boldsymbol\beta}^{ls} - \boldsymbol\beta)^\intercal (\hat{\boldsymbol\beta}^{ls} - \boldsymbol\beta)] = \sigma^2 \mathrm{tr}[(\mathbf{Z}^T \mathbf{Z})^{-1}]$$

## Mean squared error of the least squares fit

$$\mathrm{MSE}(\hat{\boldsymbol\beta}^{ls}) = \sigma^2 \mathrm{tr}[(\mathbf{Z}^T \mathbf{Z})^{-1}]$$

Interpretation:

- $\sigma^2$: the magnitude of noise in the linear model
- $(\mathbf{Z}^T \mathbf{Z})^{-1}$: Sensitivity to the design matrix. Poorly conditioned or highly correlated predictors will lead to large MSE
- $\mathrm{tr}[(\mathbf{Z}^T \mathbf{Z})^{-1}]$: Total variance across all coefficients

::: {.notes}
Recall the linear model: $\mathbf{y} = \mathbf{Z}\boldsymbol\beta + \boldsymbol\epsilon, \quad \boldsymbol\epsilon \sim \mathcal{N}(0, \sigma^2 \mathbf{I})$

The least squares estimator is: $\hat{\boldsymbol\beta}^{ls} = (\mathbf{Z}^T \mathbf{Z})^{-1} \mathbf{Z}^T \mathbf{y}$

Under these assumptions, the estimator has distribution:
$$\hat{\boldsymbol\beta}^{ls} \sim \mathcal{N}\left(\boldsymbol\beta,\; \sigma^2 (\mathbf{Z}^T \mathbf{Z})^{-1}\right)$$
...
Let $\mathbf{x} = \hat{\boldsymbol\beta}^{ls} - \boldsymbol\beta$. Since the estimator is unbiased, $\mathbb{E}[\mathbf{x}] = 0$, and $\mathrm{MSE} = \mathbb{E}[\mathbf{x}^T \mathbf{x}]$

A useful identity for zero-mean vectors:
$$\mathbb{E}[\mathbf{x}^T \mathbf{x}] = \mathrm{tr}(\mathrm{Cov}(\mathbf{x}))$$

Applying this:
$$\mathrm{MSE}(\hat{\boldsymbol\beta}^{ls}) = \mathrm{tr}\left(\mathrm{Cov}(\hat{\boldsymbol\beta}^{ls})\right)
= \sigma^2 \mathrm{tr}\left((\mathbf{Z}^T \mathbf{Z})^{-1}\right)$$
:::
...
We have the the bias-variance tradeoff:
...
![A graph depicting the bias-variance tradeoff.](./figs/regularization/bias-variance.svg){width=600 fig-alt="Graph illustrating the bias-variance tradeoff, showing how total error is minimized at an intermediate level of model complexity."}
...
$$
\begin{aligned}
\textrm{PRSS}(\beta)_{\ell_2} &= \sum_{i=1}^{n}(y_i - \mathbf{z}_i^{\top}\beta)^2 + \lambda\sum_{j=1}^{p}\beta_j^2 \\
&= (\mathbf{y} - \mathbf{Z}\beta)^{\top}(\mathbf{y} - \mathbf{Z}\beta) + \lambda{\left\|\beta\right\|}_2^2
\end{aligned}
$$

- Its solution may have smaller average PE than $\hat{\boldsymbol\beta}^{ls}$
- $\textrm{PRSS}(\boldsymbol\beta)_{l_2}$ is convex, and hence has a unique solution
- Taking derivatives, we obtain: $$\frac{\delta \textrm{PRSS}(\beta)_{l_2}}{\delta \beta} = -2\mathbf{Z}^T (y-\mathbf{Z}\beta)+2\lambda\beta$$
...
- The solution to $\textrm{PRSS}(\hat{\beta})_{l2}$ is now seen to be: $$\hat{\beta}_\lambda^{ridge} = (\mathbf{Z}^\intercal \mathbf{Z} + \lambda \mathbf{I}_p)^{-1} \mathbf{Z}^\intercal \mathbf{y}$$
...
![Ridge coefficient path for the diabetes data set found in the `lars` library in R.](./figs/regularization/ridge-path.svg){width=600 fig-alt="Plot showing the path of ridge regression coefficients as the regularization parameter lambda varies, for the diabetes dataset."}
...
![Cross validation errors from a ridge regression example on spam data.](./figs/regularization/ridge-CV.svg){width=600 fig-alt="Plot of cross-validation error versus the regularization parameter lambda for ridge regression on spam data, showing the optimal lambda value."}
...
![Estimation picture for the lasso (left) and ridge regression (right). Hastie, Tibshirani, and Friedman. Elements of Statistical Learning (2e). 2009.](./figs/regularization/estimation-lasso-ridge.png){fig-alt="Estimation picture for the lasso (left) and ridge regression (right). Elements of Statistical Learning (2nd Ed.) (c) Hastie, Tibshirani, and Friedman. 2009."}

- The residual sum of squares has elliptical contours.
- The least squares estimate is at the center of the contours.
- The diamond and the disk represents the constraint regions.
- Solutions at the corners at the diamond represent sparse solutions.

## The LASSO, LARS, and Forward Stagewise paths {.nostretch}

![Comparison of the LASSO, LARS, and Forward Stagewise coefficient paths for the diabetes data set.](./figs/regularization/lasso-algos.svg){width=600 fig-alt="Comparison of coefficient paths for LASSO, LARS, and Forward Stagewise algorithms on the diabetes dataset, showing their similarity."}
...
- Zou and Hastie (2005) propose the **elastic net**, which is a convex combination of ridge and the LASSO
	...
	- The strengths and balance of the $l_2$ (ridge) and the $l_1$ (LASSO) are both tunable hyperparameter
